...
public class EnemyCactus : EnemyFSM
{
	...
	{
		base.Start();
		enemyType = Define.EnemyType.Cactus;
	...
	protected override void EnemyHit()
	{
		if (!isHit)
		{
			Managers.Sound.Play("Effect/Enemy/CactusHitVoice");
		}
		base.EnemyHit();
		if (isAttack || isDie)
			return;
	...
	public void Init(Vector3 comeback, int level)
	{
		...
		if (level <= 0)
		{
			lev = Random.Range(1, Managers.GData.CactusBasicState.Count + 1);
		}
		else
			lev = level;

		if (lev > Managers.GData.CactusBasicState[Managers.GData.CactusBasicState.Count - 1].Level)
		{
		...
		EnemyState mystate = new EnemyState(Managers.GData.CactusBasicState[lev - 1]);
	...
	protected override void EnemyDie()
	{
		...
		Managers.Sound.Play("Effect/Enemy/CactusDie");
		Managers.GData.PlayerFeat[3].AddCount();
		...
		if (Val > 75)
		{
			...
			if (Val < 25)
				...
				Cube.GetComponent<ItemCube>().Init(6);
			else
